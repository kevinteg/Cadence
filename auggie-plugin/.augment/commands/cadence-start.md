...
argument-hint: '[pursuit|project|brainstorm|inbox]'
---
...
- **Delegated pursuits open as stubs.** `/start <pursuit>` on a pursuit with `delegated_to` frontmatter shows the stub view (why/target/win_cycle) plus a live read-only summary via `cadence delegates --json`, then points at the delegate repo for actual work. Never create projects, actions, or captures under a delegated pursuit — the CLI refuses, and the skill should not try to route around the refusal.
- **Guest sessions get the pointer, not the menu.** Outside a Cadence repo (root-resolution error / `cadence context --json` → `guest: true`), `/start` renders the guest surface: registered repos, the default target, and the note that capture + read-only status work via `--root <name>` while everything else needs a session in the target repo. No state is created.
