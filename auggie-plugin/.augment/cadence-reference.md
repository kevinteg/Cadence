...
no parallel registry view of `mcpServers`. the agent host is the MCP
...
also lingered, which was still duplicating the agent host's authority.
...
1. Use `tool discovery` to find `mcp__<server>__*` tools — if none match,
...
(the agent host's native command) — Cadence intentionally doesn't ship
its own equivalent.
...
- **A Cadence-owned MCP client / transport / registry.** Removed in
  ...
  outside of a the agent host agent (e.g., a non-Claude-Code consumer),
...
against MCP servers the agent host already knows about.
...
It declares **named queries against MCP servers the agent host already
...
An entry like `server: glean` references whatever the agent host knows
