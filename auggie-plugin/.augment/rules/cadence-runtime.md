...
**Lean into agent-host internals teaching.** When invoking a
non-obvious feature (subagents, hooks, the plugin model, MCP servers,
tool discovery, the watermark-resume narrative pattern), briefly name what
...
This is a soft-enforced guard until the the agent host Agent tool exposes
...
**Practical latency note.** MCP tool calls hop through the agent host →
