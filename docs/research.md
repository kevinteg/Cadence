...
**Bottom line up front.** Cadence's one-pager is architecturally sound for *capture, focus, and execution* — it reads like GTD + Kanban + agentic CLI, done well. Its load-bearing gap is exactly the one Kevin already named: there is no dedicated **divergent-ideation verb** before capture, and the research is unusually clear that divergent and convergent thinking are dissociable cognitive processes that corrupt each other when mixed (Guilford, 1950; Beaty et al., 2016; Diehl & Stroebe, 1987). Three other gaps fall out of the research: narrative-generation mechanics are under-specified given how much identity work the narrate verb implies (McAdams & McLean, 2013; Pennebaker, 1986); reflection is vulnerable to rumination unless explicitly scaffolded (Trapnell & Campbell, 1999; Eurich, 2017); and the LLM itself is now known to be a *convergent anchor* that homogenizes ideation (Doshi & Hauser, 2024; Anderson, Shah & Kreminski, 2024) — which has sharp implications for how the agent should be prompted across different verbs. The rest of this document is a theory-grounded map with explicit design implications in Cadence's vocabulary after each theme.
...
**The brainstorm verb must enforce pure divergent facilitation.** The verb-defined register system gives each verb its own behavioral contract. The agent during brainstorm must never critique, never evaluate, and must refuse the user's attempts to converge mid-stream ("you're in brainstorm — save that concern for develop; I'll remember it").

**Reframe Pursuit creation as a four-diamond pipeline, not a single capture.** A new Pursuit should optionally (but by default) walk through a compressed CPS/Double-Diamond arc: a Clarification phase (diverge on what the real challenge is; converge on a "How might we…?" framing), a brainstorm session (diverge-only, no evaluation, with explicit quantity goal), a develop session (PPCo — Praise, Potentials, Concerns, Overcome — on top candidates; criteria matrix to select), and only then a first execution flow that spawns Projects and Actions. Each phase invokes a distinct verb with distinct prompts and distinct behavioral contracts. Lightweight Pursuits (the "I just need to ship this feature" case) can skip through, but the full arc is the default for anything Kevin tags as strategic.

**The active verb defines the agent's register.** `/brainstorm` vs. `/start` changes the agent's behavior fundamentally. During brainstorm, the agent suppresses convergent language, rejects "is this a good idea" prompts, and holds any emerging concerns on a **Parking Lot** that surfaces only when the next develop session begins — this exploits both the Osborn rule and Masicampo–Baumeister's plan-release mechanism (writing the concern down releases the Zeigarnik tension so the ideator can return to divergence).

**Thoughts should be typed by verb context.** A Thought captured during brainstorm is a **seed**; one captured during develop is a **concern** or **criterion**; one captured during start is a **note** or **blocker**. The Reconciler should respect type and never silently promote a brainstorm seed to an Action without going through develop.
...
**Narrative generation serves competence feedback, not praise.** Drawing on Deci, Koestner & Ryan (1999), positive verbal feedback *enhances* intrinsic motivation where tangible rewards suppress it — so narrative output should describe what happened in specific, informative terms ("you unblocked the worktree issue you identified Tuesday; the Pursuit is one Project from completion") rather than in evaluative terms ("great work!"). Specific, informational feedback is the SDT-compatible register.
...
**Minimum-viable-action as a first-class starting mode.** Borrowing from behavioral activation and Fogg: when a Session opens on a Project the user has avoided for N days, the agent should offer a **two-minute entry**: the smallest possible first move. "Open the file. Read the last function. That's the Session if you want it to be." The research on action-precedes-motivation says this is often the unlock.
...
The prescriptive resolution: **WIP limits apply to convergent work-in-progress but not to divergent thinking**. A half-finished brainstorm session that generated 40 seeds is not waste; the seeds are incubating. A half-finished develop session that has three candidate designs in active evaluation *is* a WIP problem because convergent attention is being split. A half-finished execution session on code is a WIP problem in the strict Kanban sense. This distinction is not in the popular Personal Kanban literature but follows from combining the TOC logic with Beaty/Sio/Ormerod.
...
**Enforce WIP limits on Projects and convergent work, not on Pursuits and not on brainstorm.** Cadence should default to a small number — three to five — of *active* Projects (in develop or execution flow). Pursuits can number many more because they may be incubating. Brainstorm sessions accumulate Thoughts freely; their presence is not a Kanban violation.
...
**Outside-view estimation, structurally.** When the user creates an Action, the agent should prompt: *"What's the closest past Action and how long did it actually take?"* This forces reference-class framing (Flyvbjerg-style) without lecturing about the planning fallacy. The tool stores actuals and offers them on request.

**Allow ideation without penalty.** A Pursuit can sit in Clarification or brainstorm for weeks; the Reconciler should not nag. Only when a Pursuit has been in develop for more than N Sessions without convergence should the agent prompt for decision — that is a genuine convergent-WIP problem.

---

## 6. Purpose, narrative, and identity — the narrate verb
...
**Purpose lives at the Pursuit level, Golden Circle style.** Each Pursuit has optional Why/How/What fields. The agent can offer Sinek-style prompts when a Pursuit is created ("what becomes possible if this succeeds?") without making them mandatory. Autonomy-supportive, not prescriptive.
...
**The agent during brainstorm uses an Oblique-Strategies deck, not free LLM generation.** A curated set of tangential prompts (SCAMPER derivatives, "how might we" frames, forced analogies, Eno-style obliques, Six-Hats role cards) is drawn — pseudo-randomly — during brainstorm sessions. This avoids the Doshi-Hauser homogenization trap because the prompt perturbs the user's own associative search rather than generating candidate ideas directly. The deck can be extended by the user and shared across projects.

**Divergent prompts are timed and rotated.** Research on incubation (Sio & Ormerod, 2009) and the creative cliff (Lucas & Nordgren, 2020) suggests injecting a fresh provocation roughly every 8–12 minutes during a brainstorm session — specifically when fluency visibly slows — unlocks deeper categories. The agent during brainstorm should offer but not force.

**The agent during develop uses the LLM for PPCo, criteria matrix, pre-mortem, and stakeholder analysis** — tasks where LLMs are strong (structured critique and elaboration) and where their convergent bias is an asset rather than a liability.

**Divergent work happens independently first, convergent work with LLM assistance second.** At the Pursuit level: brainstorm generates Thoughts with the agent supplying prompts but *not* candidate ideas; develop then invokes the LLM to organize, cluster, critique, and select from what the user produced. This inverts the naive "just ask Claude" pattern and is directly supported by the 2024–2025 LLM homogenization literature.
...
**The active verb must be visible at all times.** The shell prompt, the TUI header, or the status line must show which verb is active (brainstorm / develop / start / reflect / etc.). This is not decoration; it is the only way the user can reliably match their own cognitive mode to the agent's register, which is what the DMN/ECN switching research says is the load-bearing capacity.
...
Cadence's one-pager covers: Pursuit > Project > Action hierarchy, Session/Marker/Thought primitives, Reflect (Get Clear + Get Focused), Narratives, Nudges, Reconciler, Leveraged Priority, verb-defined registers, two-layer architecture, local-first storage. The gaps flagged by the research:

**Brainstorm is absent as a first-class verb.** The one-pager has Sessions for execution but no named container for divergent work. Add a **brainstorm verb** whose contract is Osborn-rules (defer judgment, quantity, build-on), whose output is a Thought-cluster, and whose closure is a hand-off to develop. This is the single highest-priority gap.

**Develop is absent as a distinct verb.** The one-pager goes from capture to execution without a convergent evaluation step. Add a **develop verb** whose contract is PPCo, criteria-matrix, pre-mortem, stakeholder mapping, and whose output is a selected-and-plannable candidate.
...
**Purpose at the top of the hierarchy is not named.** Add optional Why/How/What fields at the Pursuit level. This anchors SDT autonomy and gives the narrate verb material to work with.

**The narrate verb's prompts are under-specified.** Commit to McAdams-compatible structure (what-happened / what-it-meant / what-shifted / what's-next), Eurich "what" questions in reflection, and explicit redemption-aware framing. Rule out generic praise.

**Reflect must have rumination guardrails.** The agent during reflect and reconcile should not allow Reflect to become a "why did I fail?" loop; the "what" reframe is a tool-level default.
...
**WIP limits are not differentiated by verb context.** Apply strict WIP to active Projects in develop/execution; allow unrestricted incubation during brainstorm; surface the TOC-style binding-constraint view.

**Oblique prompt deck is not yet a primitive.** Build it. It is the highest-leverage piece of infrastructure for the brainstorm verb and solves the LLM-homogenization problem structurally.
...
First, **the active verb must be explicit and ritually separated**. The Double Diamond's four phases (Discover / Define / Develop / Deliver), Puccio's four CPS phases, and de Bono's one-hat-at-a-time rule all converge on this. Cadence should never allow a Session to be simultaneously divergent and convergent; the verb determines the register, the agent behaves accordingly, and the user knows which verb is active at all times.

Second, **GTD-style capture is downstream of CPS-style ideation, not upstream**. The sequence is brainstorm → develop → (candidate emerges) → promote to Project → GTD machinery (Actions, contexts, Waiting-Fors, Weekly Review) takes over. Capture-first is correct *within* a Session, not across the lifecycle of a Pursuit. The one-pager currently treats Pursuits as if they arrive fully formed; they should arrive from CPS.

Third, **Thoughts captured during brainstorm must be held and protected, not reconciled immediately**. The Reconciler's hunger for convergence is useful during execution and dangerous during brainstorm. A Thought seeded during brainstorm belongs to a cluster, not an Action list, until develop. The tool should track Thought type (seed / concern / criterion / note / blocker) and refuse to flatten them.

Fourth, **the LLM is a convergent anchor and must be deployed accordingly**. Doshi-Hauser and Anderson-Shah-Kreminski give the empirical ground: LLMs raise the floor of individual creativity but compress its variance. Use LLMs for develop, start, and narrate — tasks where convergence and structured elaboration are the job. Use a curated Oblique-Strategies-style deck of prompts, not free LLM generation, during brainstorm.
...
Cadence's existing one-pager is two-thirds of what the research supports — it gets externalization, mode-visible flow protection, local-first autonomy, and progress-oriented feedback broadly right. The missing third is a structural commitment to the **diverge → converge → plan → execute** pipeline upstream of the existing Project/Action machinery, manifested as brainstorm and develop verbs with their own behavioral contracts, an Oblique-Strategies-style prompt deck that deliberately avoids using the LLM for divergent generation (which the 2024–2025 literature shows homogenizes output), Masicampo–Baumeister ready-to-resume fields on every Marker, Eurich-aware "what" prompts in reflect, explicit verb-context-differentiated WIP limits, and outside-view estimation that pays rent on Kahneman's planning fallacy.
