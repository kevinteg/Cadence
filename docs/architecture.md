...
**Why splash suppression rather than always firing?** The SessionStart hook fires on every new conversation. Firing the full dashboard every time would turn the surface into wallpaper. Two suppression mechanisms: a 60-minute state-hash window (no emission when nothing has changed) and an explicit `cadence dismiss-splash --hours N` window (user-driven). The bare CLI bypasses both — `cadence status` is the explicit ask.
